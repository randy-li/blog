    ...
    async def parse_data(request):
        if request.method == 'POST':
            if request.content_type.startswith('application/json'):
                ...
                logging.info('request from: {}'.format(str(request.__data__)))
        return await handler(request)
    return parse_data
...
async def response_factory(app, handler):
    async def response(request):
        ...
        if isinstance(r, bytes):
            ...
            resp.content_type = 'applicationi/octet-stream'
            return resp
        if isinstance(r, str):
...
def datetime_filter(t):
    ...
    if delta < 604800:
        return u'{} days gao'.format(delta//86400)

    dt = datetime.fromtimestamp(t)
    return u'{} years {} months {} day'.format(dt.year, dt.month, dt.day)


def index(request):
    return web.Response(body=b'<h1>Blog</h1>', headers={'content-type': 'text/html'})


async def init(loop):
    await orm.create_pool(loop, host='127.0.0.1', port=3306, user='root', password='root', db='awesome')
